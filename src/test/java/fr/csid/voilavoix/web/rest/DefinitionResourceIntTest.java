...
public class DefinitionResourceIntTest {
    ...
    @Autowired
    private DefinitionSearchRepository definitionSearchRepository;
    ...
    public void setup() {
        MockitoAnnotations.initMocks(this);
            DefinitionResource definitionResource = new DefinitionResource(definitionRepository, definitionSearchRepository);
    ...
    public void initTest() {
        definitionSearchRepository.deleteAll();
    ...
    public void createDefinition() throws Exception {
        ...
        // Validate the Definition in Elasticsearch
        Definition definitionEs = definitionSearchRepository.findOne(testDefinition.getId());
        assertThat(definitionEs).isEqualToComparingFieldByField(testDefinition);
    }
    ...
    public void updateDefinition() throws Exception {
        // Initialize the database
        definitionRepository.saveAndFlush(definition);
        definitionSearchRepository.save(definition);
    ...
    public void deleteDefinition() throws Exception {
        ...
        boolean definitionExistsInEs = definitionSearchRepository.exists(definition.getId());
        assertThat(definitionExistsInEs).isFalse();
    ...
    @Test
    @Transactional
    public void searchDefinition() throws Exception {
        // Initialize the database
        definitionRepository.saveAndFlush(definition);
        definitionSearchRepository.save(definition);

        // Search the definition
        restDefinitionMockMvc.perform(get("/api/_search/definitions?query=id:" + definition.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(definition.getId().intValue())))
            .andExpect(jsonPath("$.[*].definition").value(hasItem(DEFAULT_DEFINITION.toString())));
