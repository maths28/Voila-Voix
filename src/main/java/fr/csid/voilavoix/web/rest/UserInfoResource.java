...
import fr.csid.voilavoix.repository.UserInfoRepository;
import fr.csid.voilavoix.repository.search.UserInfoSearchRepository;
...
public class UserInfoResource {
    ...
    /**
     * GET  /user-infos/user/:id : get the "id" userInfo.
     *
     * @param login the id of the user to retrieve
     * @return the ResponseEntity with status 200 (OK) and with body the userInfo, or with status 404 (Not Found)
     */
    @GetMapping("/user-infos-user/{login}")
    @Timed
    public ResponseEntity<UserInfo> getUserInfoByUserLogin(@PathVariable String login) {
        log.debug("REST request to get UserInfoByUser : {}");
        UserInfo userInfo = null;
        //provisoire
        {

            List<UserInfo> userInfos = userInfoRepository.findAll();
            for(UserInfo anUserInfo : userInfos){
                if(anUserInfo.getUser().getLogin().equals(login)){
                    userInfo = anUserInfo ;
                }
            }
